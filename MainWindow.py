...
class mainWindow(object):
    ...
    def __initWindow(self):
        ...
        self.root.title("Android性能测试_coolqi_酷骑单车.骑乐无比")
#         BitmapImage("D:/imgs/icon.png")设置icon未成功
        ...
        self.root.iconbitmap(os.path.join(const.resPath,"z.ico"))
...
#         self.btn_end['state'] = ACTIVE
        ...
        if self.cpuCheckVar.get() == 1:
            self.cpuMonitor = memoryCPUMonitor(logFolder=self.resultFolder)
...
#             tkMessageBox.showinfo("提示", "报告已生成。请到文件夹%s下查看。"%self.path)
